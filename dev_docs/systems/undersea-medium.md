...
`sea/medium.ts` owns underwater-ness. Above the surface it is a no-op; the `submerged` uniform gates everything and is a **hard binary switch locked to the true wave-displaced waterline** (`SeaSystem.surfaceHeightAtCamera`, from the GPU waterline probe). The existing scalar uniform is written in `lateUpdate`, after player and ride camera controllers settle the pose rendered that frame; do not bind the probe storage buffer into the render graph. No smoothing: the swell dunks the camera repeatedly during the descent and each dip must swap worlds the same frame. The probe runs every frame only while the camera is within 3 m of mean sea level. Outside that conservative band it does no compute or readback; a camera must re-enter the band well before it can cross the ~0.5 m displaced surface. Crossing pops are physical — the lens-drip effect covers emergence.
