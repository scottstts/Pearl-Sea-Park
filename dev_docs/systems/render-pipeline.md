...
- `?pass=` views: `ao · ao-filtered · ao-applied · ao-mask · ao-footprint · bloom · depth · normal · exposure · rays · caustics · haze · no-rays · water-fresnel · water-reflection · water-transmission · water-interface · water-validity · water-foam · no-post · no-grade`; wake diagnostics are `wake-layers · wake-age · wake-flow`, plus the fountain field modes. `water-interface` isolates the registered Bell cage and the Arrival pavilion's clipped, tessellated air-side target. `water-foam` shows the above-water foam coverage populations unshaded (R dense whitecap/wake, G windrow raft, B crest tear). `?view=seabed-high` is the fixed high-water-column AO/minification regression camera. `?view`/`?pass` skip the enter button (validation mode).
- The ocean still has no whole-scene reflection/refraction render. Its one shadowless half-CSS auxiliary target (1024 px long-edge cap) has two bounded registrations: the Bell's live-FFT external cage and the Arrival pavilion's stable mean-plane underwater image. The pavilion source is clipped to its air-side assembly, tessellated to 1.2 m edges, rendered only for underwater cameras, and faded out from 204–240 m; it is nine material draws / about 71k triangles rather than a park replay. A bounded angular rotation of each solved apparent direction (the Snell `1 − 1/S` term, capped at half a source edge's apparent size) gives that stable image the interface's real motion without adding a pass, texture, or nonlinear Snell fold. This target exists because the pavilion's direct image can be offscreen underwater and a two-depth viewport guess repeatedly folded unrelated surfaces into a flickering cluster. The ocean performs one direct target sample, so depth never feeds back into projection. Surfaced-submarine reflection remains outside this target: its opaque exterior participates in the shared same-frame viewport reflection path at zero extra draw cost.
...
- **Loading-time warmup** (`render/warmup.ts`, awaited before the Enter
  ...
  is batch-`compileAsync`ed against the scene pass's own target + MRT. The
  actual RenderPipeline
  ...
  Validation modes still skip the submitted-frame warmup. The deleted
  undersea-radiance capture no longer contributes render, canopy, tile, mip, or
  shadow-recenter work. `canvas.dataset.loadTiming` records material
  signatures, the retained surface-shadow bake, final-pipeline compilation,
  every warm frame's submit/wall time, and full boot totals.
