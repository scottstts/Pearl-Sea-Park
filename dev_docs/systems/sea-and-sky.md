...
- **Snell's window includes real above-water geometry.** The ocean renders first in the transparent queue (while remaining alpha-opaque and depth-writing), reprojects the water→air refracted direction into a copy of the completed opaque framebuffer, and reconstructs source depth to admit only geometry above y=0 and nearer than the 3400 m sky dome. Invalid/offscreen samples, submerged geometry, and the dome fall back to the shared analytic sky + filtered window glint. Exact water→air dielectric Fresnel fades transmission to zero at the critical angle; do not replace this with a constant structure overlay or an unvalidated screen offset.
