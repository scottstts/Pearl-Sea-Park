...
- **Procedural synthesis by default** (WebAudio): music-box waltz for the carousel (composed note sequence, plucked FM voices + long reverb), calliope rag for the Midway, chime peals for the schedule, whale song (filtered noise + sine choir), and mechanical layers (gear ticks, cable hums, ratchets) synthesized per ride. Four supplied assets are deliberate exceptions in the same source graph: crossfaded `ocean_ambiance.mp3` + `seagulls.mp3` loops above water, `underwater.mp3` below water, and `water_splash.mp3` at either-direction waterline crossings.
