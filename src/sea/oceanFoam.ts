 ...
 * long-lived can live inside them, exactly as `wakeFoamMap.ts` found.
 *
...
export interface OceanFoam {
  ...
  color: Node<'vec3'>
  /** `?pass=water-foam`: R dense whitecap/wake, G windrow raft, B crest tear. */
  debug: Node<'vec3'>
}
...
export function createOceanFoam(inputs: OceanFoamInputs): OceanFoam {
  ...
  // against that scale rather than copied from the lace.
